...
?>      
        ...
        <div class="form-group">
            ...
            <div class="col-md-10 control-label tal">
                <select name="Path[technology][]" class="multi-select" multiple="" id="my_multi_select3" >
                <?php
                $technology = CHtml::listData($model->technology, 'id', 'id');                
                foreach (CHtml::listData(Technology::model()->findAll(), 'id', 'name') as $technologyId=>$technologyName)
                {
                    $selected = '';
                    if (in_array($technologyId, $technology))
                        $selected = 'selected';
                    echo '<option value="'.$technologyId.'" '.$selected.'>'.$technologyName.'</option>';
                }
                ?>
                </select>
            </div>
...
<script>
    ...
    {
        $("#Path_alias").click(function()
        {
            if($("#Path_alias").val() == '')
                $("#Path_alias").val( urlRusLat($("#Path_name").val()) );
        });
        ...
        $('#my_multi_select3').multiSelect({
            ...
            selectionHeader: "<b>Отобранные технологии</b><input type='text' class='form-control search-input' autocomplete='off' placeholder='поиск...'>",
